...
class SatelliteConfigurer():

    satellite_parameters: dict[str, tuple[str]]= {
        ""
    }

    def __init__(self, root: Tk, config_frame_placement: tuple[str], orbit_fig: OrbitFigure, orbit: Orbit, satellite: Satellite):
        self._root = root

        self._display_frame = LabelFrame(root, text = "Satellite parameters")
    ...
    def build(self) -> None:
        self._build_slider("e", "True anomaly (°)", 360, 0.01)

        self._display_frame.pack(side = "top", anchor = "nw", pady = (10, 0))

        for i, parameters in enumerate(list(self.satellite_parameters.items())):
    ...
    def _update_value(self, parameter: str, new_val: str) -> None:
        ...
        for param in self.satellite_parameters:
            self._update_display(param)

    def _build_display(self, parameter: str, display_str: str, units: str, row: int) -> None:
        var = StringVar(value = self._format_display_value(getattr(self._sat, parameter), units))
    ...
    def _update_display(self, parameter: str) -> None:
        self.__getattribute__(
            f"_{parameter}_str"
        ).set(self._format_display_value(getattr(self._sat, parameter), self.satellite_parameters[parameter][1]))
